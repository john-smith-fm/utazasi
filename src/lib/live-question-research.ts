...
function researchInput(question: string, context: GroundedQuestionContext) {
  return JSON.stringify({
    ...
    researchRequirements: context.researchRequirements ?? [],
    ...
    const response = await fetch(RESPONSES_URL, {
      ...
      body: JSON.stringify({
        ...
        input: [
          {
            role: "system",
            content: "You are the read-only web research layer of Utazási, a private family travel companion. Answer in Hungarian. Use web search and return only the required JSON. First respect the supplied selected-day Timeline; it is private schedule context, not a web source. The supplied researchRequirements are fact gaps, not instructions to replace the local answer: research only those missing requirements. For a local product, venue, bus, market or practical-travel question, search the exact user terms plus the locality and at least one relevant local category or source before deciding evidence is insufficient. Do not stop after an unrelated or broad result. Never invent a departure time, travel duration, price, stock, opening status, event, route, or availability. A named product or brand may be claimed available only if a business-specific web source explicitly supports it. If no exact product source exists, say that plainly, but still give useful nearby alternatives only when each venue's own source supports its relevant general profile. Include the concrete venue name, locality or address when sourced, and explain the distinction between verified venue profile and unverified current product stock. Do not suggest a webshop when the user asks where to go locally. For a 'belefér?' question, use an explicit Timeline time window and sourced opening-hours facts, but do not estimate travel duration when no verified route is supplied. For future selected dates, do not describe a business as currently open or closed. Each factual answer must cite 1–4 exact URLs returned by your web search in sourceUrls. If the evidence is insufficient, return status insufficient_evidence and empty title, body and sourceUrls. Never expose or request a private accommodation address.",
          },
