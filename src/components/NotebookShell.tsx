...
function SwipeRow({ label, onSelect, onDelete, disabled = false, children }: { label: string; onSelect: () => void; onDelete: () => void; disabled?: boolean; children: ReactNode }) {
  ...
  return <div className="relative overflow-hidden">
    <button type="button" disabled={disabled} onClick={onDelete} aria-label={`${label} törlése`} className="absolute inset-y-0 right-0 flex items-center justify-end rounded-l-full bg-error pr-4 text-xs font-semibold text-white disabled:opacity-50" style={{ width: Math.max(0, -offset), transition: startRef.current ? "none" : "width 200ms ease-out" }}>Törlés</button>
...
export function NotebookShell() {
  ...
  async function addPacking(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!packingTitle.trim()) return; try { const current = dataRef.current; const item = await request("POST", { resource: "packing", data: { title: packingTitle.trim(), isPacked: false, position: current.packing.length } }) as unknown as PackingItemRecord; setNotebook({ ...current, packing: [item, ...current.packing] }); setPackingTitle(""); setMessage(null); } catch (error) { setMessage(error instanceof Error ? error.message : "A mentés nem sikerült."); } }
