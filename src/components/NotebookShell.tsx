...
export function NotebookShell() {
  ...
  return <section className="mt-6" aria-label="Jegyzetfüzet tartalma">
    ...
    <div key={tab} role="tabpanel" aria-label={TABS.find((item) => item.id === tab)?.label} className={`folder-content-surface ${activeTabIndex === 0 ? "folder-content-surface--first" : ""} ${activeTabIndex === TABS.length - 1 ? "folder-content-surface--last" : ""} ${transition === "forward" ? "view-transition-forward" : "view-transition-backward"}`}>
