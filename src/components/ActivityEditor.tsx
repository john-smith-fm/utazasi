  ...
  return <section role="dialog" aria-modal="true" aria-label={editing ? "Program szerkesztése" : "Új program"} className="fixed inset-0 z-[70] overflow-y-auto bg-quartz text-deep-sea">
    <form onSubmit={(event) => void submit(event)} className="mx-auto flex min-h-dvh max-w-[430px] flex-col px-5 pb-[calc(28px+env(safe-area-inset-bottom))] pt-[env(safe-area-inset-top)]">
      ...
      <div className="mt-8 space-y-5">
        <Field label="Program"><input autoFocus required maxLength={120} value={input.title} onChange={(event) => update("title", event.target.value)} className="h-12 w-full rounded-ui-s border border-deep-sea/10 bg-white px-4 text-[16px] outline-none focus:border-turquoise-dark" /></Field>
        <div className="grid grid-cols-2 gap-3">
          <Field label="Kezdés"><input required type="time" value={input.startTime} onChange={(event) => update("startTime", event.target.value)} className="h-12 w-full rounded-ui-s border border-deep-sea/10 bg-white px-3 text-[16px] outline-none focus:border-turquoise-dark" /></Field>
          <Field label="Időtartam (perc)"><input required type="number" min="1" max="1440" inputMode="numeric" value={input.durationMinutes} onChange={(event) => update("durationMinutes", Number(event.target.value))} className="h-12 w-full rounded-ui-s border border-deep-sea/10 bg-white px-3 text-[16px] outline-none focus:border-turquoise-dark" /></Field>
        </div>
        <Field label="Helyszín">
          <div className="relative">
            <input
              ...
              className="h-12 w-full rounded-ui-s border border-deep-sea/10 bg-white px-4 text-[16px] outline-none focus:border-turquoise-dark"
            />
        ...
        <Field label="Megjegyzés"><textarea maxLength={1000} value={input.description} onChange={(event) => update("description", event.target.value)} className="min-h-28 w-full resize-y rounded-ui-s border border-deep-sea/10 bg-white px-4 py-3 text-[16px] leading-[21px] outline-none focus:border-turquoise-dark" /></Field>
      </div>
