...
export function Hero() {
  const [startYear, , startDay] = TRIP_CORE.dates.start.split("-");
  const endDay = TRIP_CORE.dates.end.slice(-2);
  const tripDate = `${startYear}. szept. ${Number(startDay)}–${Number(endDay)}.`;
  ...
  return <header className="relative overflow-hidden bg-[#2f6970] pt-[env(safe-area-inset-top)] text-white" style={{ height: "calc(204px + env(safe-area-inset-top))", marginTop: "calc(-1 * env(safe-area-inset-top))" }}>
    ...
    <div className="relative flex h-full items-center justify-between gap-6 px-5">
      ...
      <div className="min-w-0"><p className="text-sm font-bold leading-[19px] text-white/95">{TRIP_CORE.destination.region}</p><p className="text-sm font-medium leading-[19px] text-white/95">{tripDate}</p></div>
    </div>
  </header>;
}
