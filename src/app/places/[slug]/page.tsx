...
import { isPlaceBrowseCategory, placeBrowseCategoryForType, placeBrowseHref } from "@/lib/place-categories";
...
export default function PlaceDetailPage({ params, searchParams }: { params: { slug: string }; searchParams?: { category?: string } }) {
  ...
  if (!place) notFound();
  const category = isPlaceBrowseCategory(searchParams?.category)
    ? searchParams.category
    : placeBrowseCategoryForType(place.type);
