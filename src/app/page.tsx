...
export default function HomePage() {
  ...
  return <><Hero /><main className="relative z-10 mx-auto max-w-[430px] overflow-x-hidden pb-[126px]"><div className="px-5"><StatRow /></div><SunCard /><div className="px-5"><TimelineCard day={day} onSelect={setSelectedDate} /><section className="mt-7"><div className="mb-5"><p className="text-xs font-semibold tracking-[.015em] text-deep-sea/55">Mai program</p><h2 className="mt-1 text-[22px] font-bold leading-7 tracking-[-.025em]">A nap története</h2></div><PlanList activities={activities} onEdit={setEditing} /></section></div></main><button type="button" aria-label="Program hozzáadása" onClick={() => setEditing({ time: "", title: "Új programpont", place: "" })} className="fixed bottom-[calc(88px+env(safe-area-inset-bottom))] right-5 z-[55] grid h-[54px] w-[54px] place-items-center rounded-full bg-coral text-deep-sea shadow-[0_12px_28px_rgba(217,99,57,.28)] transition-transform active:scale-95"><Icon name="plus" size={24} strokeWidth={2} /></button>
