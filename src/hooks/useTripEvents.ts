...
import { tripEventsDayCacheKey } from "@/lib/trip-cache";
...
export function useTripEvents(tripSlug: string, date: string) {
  const [loaded, setLoaded] = useState<{ date: string; events: TripEvent[] }>({ date: "", events: [] });
  useEffect(() => {
    let active = true;
    const key = tripEventsDayCacheKey(tripSlug, date);
    ...
    void fetch(`/api/events?trip=${encodeURIComponent(tripSlug)}&date=${encodeURIComponent(date)}`, { cache: "no-store" })
